...
class AdminSlidersController extends Controller
{
	...
	public function sliders(Request $request){
		...
		$banners = DB::table('sliders_images')->leftJoin('languages','languages.languages_id','=','sliders_images.languages_id')->orderBy('sliders_images.sliders_id','DESC')->paginate(20);
	...
	public function addNewSlide(Request $request){
		...
		if (!file_exists('assets/images/slider_images/')) {
			mkdir('assets/images/slider_images/', 0777, true);
		}
		
		if($request->hasFile('newImage') and in_array($request->newImage->extension(), $extensions)){
			$image = $request->newImage;
			$fileName = time().'.'.$image->getClientOriginalName();
			$image->move('assets/images/slider_images/', $fileName);
			$uploadImage = 'assets/images/slider_images/'.$fileName; 
		}else{
			$uploadImage = '';
		}
	...
	public function updateSlide(Request $request){
		...
		if($request->hasFile('newImage') and in_array($request->newImage->extension(), $extensions)){
			$image = $request->newImage;
			$fileName = time().'.'.$image->getClientOriginalName();
			$image->move('assets/images/banner_images/', $fileName);
			$uploadImage = 'assets/images/banner_images/'.$fileName; 
		}else{
